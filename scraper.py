"""
TecVagas - Robo de Vagas v13 PREMIUM
Tema premium + disjuntor light/dark + botao engrenagens + mapa funcional + pizza premium
"""
...
def extrair_cidade(local_str):
    ...
    texto = texto.strip(" ,-/")
    if texto.lower() in ["brasil", "brazil", "remoto", "remote", "hibrido", "híbrido"]:
        return ""
    return texto[:40]
    ...
    for termo in TECNICOS[:25]:
        try:
            ...
            for job in resp.json().get("data", []):
                ...
                local_display = montar_local(cidade, estado, local_combinado)
...
def verificar_cache(cache):
    ...
    for v in cache:
        ...
        if not v.get("estado") and v.get("local"):
...
<defs>
  <radialGradient id="mapBgGlow" cx="50%" cy="38%">
    <stop offset="0%" stop-color="rgba(79,140,255,.18)"/>
...
<svg viewBox="0 0 220 220" class="pie-chart">
  <defs>
    <radialGradient id="pieCoreGlow" cx="50%" cy="50%">
      ...
      <stop offset="100%" stop-color="rgba(79,140,255,.15)"/>
    </radialGradient>
  </defs>
  {segmentos}
  <circle cx="{cx}" cy="{cy}" r="40" fill="var(--pieCore)"/>
  <circle cx="{cx}" cy="{cy}" r="32" fill="url(#pieCoreGlow)" opacity=".22"/>
  <text x="{cx}" y="{cy-3}" text-anchor="middle" font-size="24" font-weight="800" fill="var(--pieText)" font-family="Syne,sans-serif">{total}</text>
...
--glass2:rgba(15,23,42,.08);
--line:rgba(15,23,42,.08);
--line2:rgba(79,140,255,.18);
--text:#0f172a;
--text2:#1e293b;
--muted:#64748b;
--muted2:#475569;
...
body{
background:
radial-gradient(circle at top center, rgba(79,140,255,.10), transparent 28%),
...
background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(79,140,255,.06));
...
.breaker{
width:54px;height:74px;border-radius:14px;
...
.breaker:hover{transform:translateY(-1px)}
.breaker-slot{
width:22px;height:48px;border-radius:10px;
background:rgba(0,0,0,.18);border:1px solid var(--line);
position:relative;overflow:hidden
}
.breaker-handle{
position:absolute;left:2px;top:3px;
width:16px;height:20px;border-radius:7px;
background:linear-gradient(180deg,#ffffff,#cfe0ff);
box-shadow:0 6px 16px rgba(79,140,255,.18);
transition:all .25s ease
}
body.dark-mode .breaker-handle{top:25px;background:linear-gradient(180deg,#bfd7ff,#7faeff)}
.breaker-label{
...
.gear-power{
height:44px;padding:0 14px;border-radius:999px;border:1px solid rgba(255,77,79,.28);
background:linear-gradient(180deg,rgba(255,77,79,.18),rgba(255,77,79,.10));
color:#ffd9da;font-size:11px;font-weight:800;letter-spacing:.8px;
cursor:pointer;display:flex;align-items:center;gap:8px;
transition:all .25s ease
}
.gear-power .gear-power-dot{
width:10px;height:10px;border-radius:50%;background:var(--danger);
box-shadow:0 0 12px var(--danger)
}
body.gears-on .gear-power{
border-color:rgba(41,196,138,.28);
background:linear-gradient(180deg,rgba(41,196,138,.18),rgba(41,196,138,.10));
color:#d9ffef
}
body.gears-on .gear-power .gear-power-dot{
...
background:
radial-gradient(ellipse 34% 38% at 50% 15%, rgba(255,255,255,.18) 0%, rgba(219,234,254,.11) 18%, rgba(79,140,255,.12) 40%, rgba(79,140,255,.08) 56%, transparent 76%),
radial-gradient(ellipse 58% 75% at 50% 0%, rgba(79,140,255,.10) 0%, transparent 72%);
pointer-events:none;filter:blur(7px)
}
.hero-gear,.section-gear,.footer-gear{
position:absolute;opacity:0;pointer-events:none;transition:opacity .28s ease
}
body.gears-on .hero-gear, body.gears-on .section-gear, body.gears-on .footer-gear{opacity:.09}
...
.section-gear.one{left:-20px;top:120px;width:160px;height:160px;opacity:0}
.section-gear.two{right:-20px;top:30px;width:130px;height:130px;opacity:0}
.footer-gear{left:50%;transform:translateX(-50%);bottom:12px;width:140px;height:140px;opacity:0}

.hero-eyebrow{
display:inline-flex;align-items:center;gap:8px;
font-size:11px;font-weight:800;letter-spacing:1.6px;
color:var(--text);margin-bottom:24px;
background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(79,140,255,.05));
border:1px solid var(--line2);
padding:8px 16px;border-radius:999px;
box-shadow:0 12px 38px rgba(79,140,255,.12), inset 0 1px 0 rgba(255,255,255,.08);
...
color:var(--text);margin-bottom:22px;max-width:920px;
...
body.light-mode .hero h1 .accent{
background:linear-gradient(180deg,#0f172a 0%, #17365f 18%, #2563eb 58%, #60a5fa 100%);
-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text
}
.hero-sub{
font-size:17px;color:var(--muted2);font-weight:500;
max-width:560px;margin:0 auto;line-height:1.7;
text-shadow:0 6px 20px rgba(79,140,255,.08)
}
...
.metric-num{font-family:'Syne',sans-serif;font-size:34px;font-weight:800;color:var(--text);line-height:1;margin-bottom:6px;letter-spacing:-1.6px}
...
.chip{
flex-shrink:0;background:var(--glass);border:1px solid var(--line);
...
.chip.active{
background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(79,140,255,.10));
...
.trust-bar{
background:linear-gradient(90deg,rgba(79,140,255,.05),rgba(255,255,255,.02));
...
background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(79,140,255,.08));
border:1px solid var(--line2);
...
.dash-card::before{
content:'';position:absolute;top:0;left:0;right:0;height:2px;
background:linear-gradient(90deg,transparent,rgba(79,140,255,.85),transparent)
}
...
background:color-mix(in srgb, var(--bg2) 85%, transparent);border:1px solid var(--line2);
padding:6px 10px;border-radius:10px;font-size:10px;color:var(--text2);pointer-events:none;font-weight:700
}
...
padding:8px 10px;border-radius:12px;background:var(--glass);border:1px solid var(--line)
}
...
.article-title{font-size:14px;font-weight:700;color:var(--text);margin-bottom:4px;line-height:1.35}
...
.newsletter{
background:
linear-gradient(135deg,rgba(255,255,255,.06),rgba(79,140,255,.06)),
linear-gradient(180deg,var(--bg2),var(--bg1));
border:1px solid var(--line2);border-radius:24px;padding:26px 20px;margin-bottom:4px;position:relative;overflow:hidden
}
.newsletter::before{
content:'';position:absolute;top:0;left:0;right:0;height:3px;
background:linear-gradient(90deg,#ffffff,#84b6ff,#4f8cff,#ffffff);background-size:200% 100%;animation:slide 4s linear infinite
}
...
.nl-btn{
background:linear-gradient(180deg,#dbeafe,#4f8cff);color:#06111f;border:none;font-family:'DM Sans',sans-serif;
...
.job-card:hover{
border-color:var(--line2);background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.04));
...
.card-badge-verified{
flex-shrink:0;font-size:10px;font-weight:700;color:var(--text);
background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(79,140,255,.08));
border:1px solid var(--line2);padding:4px 10px;border-radius:999px;white-space:nowrap
}
.card-info-row{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:11px}
.info-chip{
font-size:10px;color:var(--muted2);background:var(--glass);border:1px solid var(--line);
...
background:rgba(41,196,138,.08);border:1px solid rgba(41,196,138,.18);padding:8px 13px;border-radius:14px;text-decoration:none;transition:all .2s
}
...
background:linear-gradient(180deg,#dbeafe,#4f8cff);padding:8px 15px;border-radius:14px;text-decoration:none;transition:all .2s;margin-left:auto
}
...
background:linear-gradient(180deg,#dbeafe,#84b6ff);border:none;padding:10px 20px;border-radius:999px;cursor:pointer;transition:all .2s
}
...
.modal-box{
background:linear-gradient(180deg,var(--bg2),var(--bg1));border:1px solid var(--line2);
border-radius:24px;padding:22px;width:100%;max-width:420px;margin:auto;position:relative
}
.modal-box::before{
content:'';position:absolute;top:0;left:0;right:0;height:2px;
background:linear-gradient(90deg,#ffffff,#84b6ff,#4f8cff)
}
...
.modal-article-title{font-family:'Syne',sans-serif;font-size:22px;font-weight:800;line-height:1.2;margin-bottom:8px;letter-spacing:-.4px;color:var(--text)}
...
.modal-title{font-family:'Syne',sans-serif;font-size:18px;font-weight:700;color:var(--text);margin-bottom:14px}
...
nav{height:auto;padding-top:10px;padding-bottom:10px;align-items:flex-start}
...
.breaker{width:48px;height:68px}
.gear-power{height:40px;padding:0 12px}
}
"""
...
(function(){
  ...
  function rotateByScroll(){
    if(!body.classList.contains('gears-on')) return;
    var y=window.scrollY;
    document.querySelectorAll('.hero-gear').forEach(function(g){
      g.style.transform='rotate('+(y*0.12)+'deg)';
    });
    document.querySelectorAll('.section-gear.one').forEach(function(g){
      g.style.transform='rotate('+(y*0.08)+'deg)';
    });
    document.querySelectorAll('.section-gear.two').forEach(function(g){
      g.style.transform='rotate('+(-y*0.06)+'deg)';
    });
    document.querySelectorAll('.footer-gear').forEach(function(g){
      g.style.transform='translateX(-50%) rotate('+(y*0.1)+'deg)';
    });
  }

  window.addEventListener('scroll',rotateByScroll,{passive:true});
  rotateByScroll();
})();
"""
...
def gerar_html(vagas, artigos):
    ...
    out += '<style>' + HTML_CSS + '</style></head><body class="dark-mode">'
    ...
    out += '<div class="breaker" id="theme-breaker" title="Trocar tema"><div class="breaker-slot"><div class="breaker-handle"></div></div><div class="breaker-label">TEMA</div></div>'
    out += '<button class="gear-power" id="gear-toggle" title="Ligar engrenagens"><span class="gear-power-dot"></span>ENGRENAGENS</button>'
    out += '</div></nav>'
...
def main():
    print("\nTecVagas v13 PREMIUM - " + datetime.now().strftime("%d/%m/%Y %H:%M"))
    print("=" * 50)
